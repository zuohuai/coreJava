...
public final class JsonUtils {
	...
	public static <T> T string2Object(String json, Type type) {
		try {
			...
			FormattingTuple message = MessageFormatter.format(
					"将字符串[{}]转换为对象[{}]时出现异常", new Object[] { json, type, e });
			throw new RuntimeException(message.getMessage(), e);
		}
	}
	 ...
	 * 
	 * @param tr
	 *            示例 new TypeReference<List<Long>>(){}
	 **/
	public static <T> T string2GenericObject(String json, TypeReference<T> tr) {
		if (StringUtils.isBlank(json)) {
			...
			try {
				...
				FormattingTuple message = MessageFormatter.format(
						"将字符串[{}]转换为[{}]时出现异常", new Object[] { json, tr });
				throw new RuntimeException(message.getMessage(), e);
			}
		}
	}
	...
	public static <C extends Collection<E>, E> C string2Collection(
		...
		try {
			...
			FormattingTuple message = MessageFormatter.format(
					"将字符串[{}]转换为集合[{}]时出现异常", new Object[] { content,
	...
	public static <T> T convertObject(Object value, Type type) {
		try {
			...
			FormattingTuple message = MessageFormatter.format(
					"将对象[{}]转换为类型[{}]时出现异常", new Object[] { value, type, e });
			throw new RuntimeException(message.getMessage(), e);
		}
	}
