...
import com.edu.utils.math.MathUtils;
import com.edu.utils.math.RandomUtils;;

/**
 * 人品(RP = Ratio of Probability)对象
 * @author Frank
 */
public class RatioProbability2<T> {

	/** 概率与结果的映射关系 */
	private LinkedHashMap<Key, T> odds = new LinkedHashMap<Key, T>();

	/** 构造器 */
	private RatioProbability2(Map<T, Integer> values) {
		double total = 0.0;
	...
	/** 构造器 */
	private RatioProbability2(T[] values, int[] weight) {
	...
	/** 构造方法 */
	public static <T> RatioProbability2<T> valueOf(Map<T, Integer> values) {
		return new RatioProbability2<T>(values);
	}

	/** 构造方法 */
	public static <T> RatioProbability2<T> valueOf(T[] values, int[] weight) {
		return new RatioProbability2<T>(values, weight);
	}
