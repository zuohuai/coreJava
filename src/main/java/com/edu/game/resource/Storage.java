...
import com.edu.game.resource.other.Getter;
import com.edu.game.resource.other.GetterBuilder;
import com.edu.game.resource.other.IndexGetter;
import com.edu.game.resource.other.InjectDefinition;
...
public class Storage<K, V> extends Observable implements ApplicationContextAware {
	...
	public synchronized void initialize(ResourceDefinition definition) {
		...
		if (logger.isDebugEnabled()) {
			String info = String.format("加载 %s  时间: %.2f 秒 数据: %d", resourceDefinition.getLocation(), (end - start) * 0.001, this.values.size());
			logger.debug(info);
		}
	}
